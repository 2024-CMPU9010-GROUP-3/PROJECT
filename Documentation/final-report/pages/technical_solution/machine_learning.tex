...
The classification is evaluated on the following metrics: Average Intersection over Union (IoU), Balanced and Accuracy, Precision, Recall, F1 Score, Accuracy, Specificity per class. These metrics are saved in a csv file for each test image as well as the overall average metrics on the entire test set.
...
The overall results on test set are presented below in Table~\ref{tab:metrics}
...
Analyse metrics + Mention common problems.

Some images from the test set can be seen below, in Table~\ref{tab:images}
