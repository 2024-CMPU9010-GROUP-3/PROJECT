...
\paragraph{}\subsubsection{Prototyping}
...
\subsubsection{User Interface Iteration}
...
\subsubsection{User Feedback and Prototype Evolution}

User feedback plays a key guiding role in all stages of prototype design. We have established a systematic feedback collection and analysis framework to ensure that the prototype can be continuously optimized according to actual user needs.

\textbf{Feedback collection method}
\begin{itemize}
\item \textbf{User interview}
\begin{itemize}
\item Conduct one-on-one in-depth interviews with target users
\item Collect qualitative feedback from users on their intuitive experience of the interface
\item Understand the specific problems users encounter during use
\end{itemize}

\item \textbf{Usability testing}
\begin{itemize}
\item Design specific task scenarios for testing
\item Observe the behavioral path of users completing tasks
\item Record the confusion points and jams when users operate
\end{itemize}

\item \textbf{Questionnaire}
\begin{itemize}
\item Collect quantitative feedback using the Likert scale
\item Set open-ended questions to obtain suggestions
\item Statistically analyze user satisfaction data
\end{itemize}
\end{itemize}
...
The second iteration focused on enhancing the core functionality and user interaction patterns. A major addition was the introduction of the search bar, initially placed at the top of the screen. However, user testing revealed that this placement, while conventional, didn't align optimally with the workflow of our specific application. The filter system, implemented as checkboxes, proved to be less intuitive than anticipated, with users expressing a desire for more immediate visual feedback during their interactions.
